...
const Loginpage = (props) => {
			...
			Axios.post(process.env.REACT_APP_API_URL + "/api/login", formData)
				.then((response) => {
					...
					if (response.data.user.roles.status === true || response.data.user.roles.status === "active") {
						...
						history.push("/app/main/dashboard");
						Axios.get(process.env.REACT_APP_API_URL + `/api/userRole/id/list?id=${response.data.user.role}`).then(
							(res) => {
	...
	return (
		<>
			<Helmet>
				<title>Login - Sheranwala Group</title>
							...
							<div>
								...
								>
									...
									<div className="form-group text-center">
										<button className="btn btn-primary account-btn" type="submit" onClick={submit} disabled={loading}>
											Login
										</button>
									</div>
								</form>
