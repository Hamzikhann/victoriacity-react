...
const employeesalaryhistory = () => {
	...
	const [isShowEmployeeSalaryHistoryModal, setIsShowEmployeeSalaryHistoryModal] = useState(false);
	const [isEditEmployeeSalaryHistoryModal, setIsEditEmployeeSalaryHistoryModal] = useState(false);
	const [query, setQuery] = useState("");
	const [baseApiUrl, setBaseApiUrl] = useState(process.env.REACT_APP_API_URL + "/api/");
	const [employeeSalaryHistoryInitialValues, setEmployeeSalaryHistoryInitialValues] = useState({
		employeeId: "",
		salary: "",
		fullName: ""
	});
					...
					<div className="modal-body">
						<Formik
							...
							onSubmit={async (values, { setSubmitting }) => {
								...
								try {
									setLoading(true);
									const res = await Axios.post(baseApiUrl + "employeeSalaryHisory/add", formData);
									if (res.data.status == 200) {
										getAllEmployeeSalary();
										toast.success(res.data.message);
										setLoading(false);
										...
										<div className="row">
											<div className="col-sm-12">
												<div className="form-group">
													...
													/>
													<span className="error">{errors.employeeId && touched.employeeId && errors.employeeId}</span>
												</div>
											</div>
											...
											<div className="col-sm-12">
												<div className="form-group">
													...
													/>
													<span className="error">{errors.salary && touched.salary && errors.salary}</span>
												</div>
											</div>
										</div>
					...
					<div className="modal-body">
						<Formik
							...
							onSubmit={async (values, { setSubmitting }) => {
								...
								try {
									setLoading(true);
									const res = await Axios.put(baseApiUrl + `employeeSalaryHisory/update?id=${query}`, formData);
