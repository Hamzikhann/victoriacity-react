                  ...
                  <div className="px-4 p-1accordion-content">
                    {props.data?.children?.length > 0 &&
                      props.data?.children?.map((item) => (
                        <CategoryDisplay
                          data={item}
                          setIsShowEditCategoryModal={
                            props.setIsShowEditCategoryModal
                          }
...
const ExpenseCategory = () => {
  ...
  const getExpenseCategory = () => {
    ...
    Axios.get(baseApiUrl + "expenseCategory/all/list")
      .then((res) => {
        if (res.data?.data) {
          setData(res.data?.data);
  ...
  useEffect(() => {
    // getExpenseCategory();
    getExpenseCategoryid(projectId);
  }, []);
  ...
  const addCategoryHandle = (e) => {
    ...
    Axios.post(baseApiUrl + "expenseCategory/add", formData)
      .then((res) => {
        getExpenseCategoryid(projectId);
        setProjectId(id);
        setCategoryName("");
  ...
  const updateCategoryHandle = (e) => {
    ...
    Axios.put(baseApiUrl + `expenseCategory/update?id=${updateId}`, formData)
      .then((res) => {
        // getExpenseCategory();
        getExpenseCategoryid(projectId);
        setEditCategoryName("");
  ...
  const deleteExpenseCategoryById = (id) => {
    Axios.delete(baseApiUrl + `expenseCategory/delete?id=${id}`)
      .then((res) => {
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <form>
                <div className="form-group">
                  <label>
                    Expense Category Name <span className="text-danger">*</span>
                  </label>
                  <input
                    className="form-control"
                    type="text"
                ...
                <div className="form-group">
                  ...
                  <DropdownTreeSelect
                    ...
                    onNodeToggle={onNodeToggle}
                    className="w-100"
                    // onChange={(currentNode) => setParentCategory(currentNode)}
                    onChange={(currentNode) => {
                      if (currentNode.checked) {
                  ...
                  <div className="row">
                    <div className="col-6">
                      <button
                        ...
                        type="submit"
                        onClick={() => deleteExpenseCategoryById(query)}
                      >
                        Delete
                      </button>
                    </div>
