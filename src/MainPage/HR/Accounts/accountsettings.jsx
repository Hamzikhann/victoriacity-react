...
const AccountSettings = () => {
  ...
  const [isShowAccountSettingModal, setIsShowAccountSettingModal] =
    useState(false);
  const [isEditAccountSettingModal, setIsEditAccountSettingModal] =
    useState(false);

  const [accountSettingInitialValues, setAccountSettingInitialValues] =
    useState({
      incomeCategoryId: "",
      expenseCategoryId: "",
    });
            ...
            <Formik
              ...
              onSubmit={async (values, { setSubmitting }) => {
                ...
                try {
                  setLoading(true);
                  const res = await Axios.post(
                    baseApiUrl + `settings/update?id=${query}`,
                    formData
                  );
                  if (res.data.status == 200) {
                    getAllSetting();
