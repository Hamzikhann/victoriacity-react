...
const AttendanceAdmin = () => {
				...
				return (
					<Form.Item name={`attendance`}>
						...
						>
							...
							<Radio value={`p-${record?.employee?.employeeId}`} key={`p-${record?.employee?.employeeId}`}>
								Present
							</Radio>
							<Radio value={`a-${record?.employee?.employeeId}`} key={`a-${record?.employee?.employeeId}`}>
								Absent
							</Radio>
							<Radio value={`l-${record?.employee?.employeeId}`} key={`l-${record?.employee?.employeeId}`}>
								Leave
							</Radio>
	...
	const getAllEmployee = () => {
		Axios.get(baseApiUrl + "employee/list").then((res) => setAllEmployee(res.data.employee));
	};
						...
						<Form form={form} onFinish={getAttendance || getSpecificEmployee}>
							<div className="row filter-row">
								...
								<div className="col-sm-6 col-md-3">
									<div className="form-group">
										<label>Search</label>
										<Button className="text-bg-success text-light" block onClick={getSpecificEmployee}>
											Search
										</Button>
									</div>
								</div>
