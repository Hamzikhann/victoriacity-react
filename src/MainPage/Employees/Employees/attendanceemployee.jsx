...
const AttendanceEmployee = () => {
	...
	const getAttendanceByEmpID = () => {
		Axios.get(
			baseApiUrl + `attendance/employeeId?employeeId=${object.employee.id}&date=${DATE}&year=${YEAR}&month=${MONTH + 1}`
		)
			.then((res) => {
							...
							<div className="card punch-status">
								<div className="card-body">
									<h5 className="card-title">
										Timesheet <small className="text-muted">{DATE + ` ` + monthNames[MONTH] + ` ` + YEAR}</small>
									</h5>
									{visibility && (
