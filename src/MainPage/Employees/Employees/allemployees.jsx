...
const AllEmployees = () => {
	...
	const handleSelectedOption = (value) => {
		...
		setSelectedEmployees(allEmployee.filter((emp) => emp.designation === value));
	};
						...
						{designationTitle
							? selectedEmployees
									.filter((employee) => {
										if (query === "") {
											return employee;
										} else if (employee.fullName.toLowerCase().includes(query.toLowerCase())) {
											return employee;
										}
									})
									.map((employee, i) => (
											...
											<div className="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3">
												<div className="profile-widget">
													...
													<h4 className="user-name m-t-10 mb-0 text-ellipsis">
														<Link to={`/app/profile/employee-profile/?id=${employee.id}`} key={i + employee.id}>
															{employee && employee.fullName}
														</Link>
													</h4>
													<div className="small text-muted">
														{employee && employee.designationAss && employee.designationAss.title}
													</div>
												</div>
											</div>
										</>
										...
										<>
											...
											<div className="col-md-4 col-sm-6 col-12 col-lg-4 col-xl-3">
												<div className="profile-widget">
													...
													<div className="small text-muted">{employee?.designationAss?.title}</div>
												</div>
											</div>
										</>
									))}
						...
						<div className="modal-body" style={{ textAlign: "left" }}>
							<Formik
								...
								onSubmit={async (values, { setSubmitting }) => {
									...
									try {
										setloading(true);
										const res = await Axios.post(baseApiUrl + "employee/add", formData, header);
										if (res.data.status == 200) {
											setloading(false);
											toast.success(res.data.message);
									...
									<form onSubmit={handleSubmit}>
										<div className="row">
											...
											<div className="col-sm-6">
												<div className="form-group">
													...
													<span className="error">
														{errors.father_name && touched.father_name && errors.father_name}
													</span>
												</div>
											</div>
											<div className="col-sm-6">
											...
											<div className="col-sm-6">
												<div className="form-group">
													<label className="col-form-label">
														Current Designation <span className="text-danger">*</span>
													</label>
													<Select
														placeholder="Select Designation"
														options={designation}
