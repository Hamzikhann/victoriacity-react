							...
							<Link
								...
								onClick={() => {
									...
									setGroupsInitialValues({
										...
										GROUP_DESCRIPTION: "",
										...text,
										Menu: menu.find((item) => item.value === text.Menu?.Menu_ID)
									});
								}}
							>
		...
		Axios.get(baseApiUrl + "menu/list")
			.then((res) => {
				res.data.Menu.map((item) => {
					setMenu((prev) => [...prev, { label: item?.Title, value: item?.Menu_ID }]);
				});
				setTotalPage(res.data.totalPage);
			})
			.catch((err) => console.log(err.response.data));
				...
				<div className="page-header">
					<div className="row align-items-center">
						...
						<div className="col-auto float-end ml-auto">
							<p className="btn add-btn button" onClick={() => setIsShowGroups(true)}>
					...
					<div className="modal-body">
						<Formik
							...
							onSubmit={async (values, { setSubmitting }) => {
								...
								try {
									setloading(true);
									const res = await Axios.post(baseApiUrl + "/userGroup/add", formData);
									if (res.data.status == 200) {
										getAllGroup();
										toast.success(res.data.message);
										setIsShowGroups(false);
										...
										<div className="row">
											<div className="col-sm-12">
												<div className="form-group">
													...
													<span className="error">{errors.GROUP_NAME && touched.GROUP_NAME && errors.GROUP_NAME}</span>
												</div>
											</div>
											<div className="col-sm-12">
												<div className="form-group">
													...
													<span className="error">
														{errors.GROUP_DESCRIPTION && touched.GROUP_DESCRIPTION && errors.GROUP_DESCRIPTION}
													</span>
												</div>
											</div>
											<div className="col-sm-12">
												<div className="form-group">
													...
													<span className="error">{errors.menus && touched.menus && errors.menus}</span>
												</div>
											</div>
										</div>
					...
					<div className="modal-body">
						<Formik
							...
							onSubmit={async (values, { setSubmitting }) => {
								...
								try {
									setloading(true);
									const res = await Axios.put(baseApiUrl + `userGroup/update?id=${query}`, formData);
									if (res.data.status == 200) {
										getAllGroup();
										toast.success(res.data.message);
										setIsUpdateGroups(false);
										...
										<div className="row">
											...
											<div className="col-sm-12">
												<div className="form-group">
													...
													<Select
														options={menu}
														value={menu?.find((item) => item?.value == values?.Menu?.value)}
														// value={selectedOptions}
														// onChange={handleSelectChange}
														onChange={(selectedItems) => {
															handleSelectChange();
