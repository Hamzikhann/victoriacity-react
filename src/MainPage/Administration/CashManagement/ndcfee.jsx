				...
				return (
					<div className="dropdown dropdown-action text-end">
						...
						<div className="dropdown-menu dropdown-menu-right">
								...
								<>
									<Link
										...
										onClick={async () => {
											...
											const res = await Axios.put(baseApiUrl + `ndc/update/status?id=${record.TRSR_ID}`, formData);
							...
							{(userRole?.slug == "csr_manager" || userRole?.name == "Admin") && (
								<>
							...
							{text.status == 1 && (
								...
								>
									<i className="fa fa-download m-r-5" /> Dispatch To Accounts Depart
								</Link>
							)}
							...
							{text.status == 2 && (userRole?.slug == "accounts" || userRole?.slug == "cashier") && (
								<>
									...
									>
										<i className="fa fa-download m-r-5" /> Dispatch To CSR Depart
									</Link>
									{record?.BUYER_MEMBER_ID ? (
										...
										<Link
											...
											onClick={() => {
												...
												const MemberData = getAllMembersbyId(record.BUYER_MEMBER_ID);
										...
										<Link
											...
											onClick={async () => {
												...
												setNomineeInitialValues((prev) => ({
													...prev,
													MEMBER_ID: memberList.find((item) => item.id == record.BUYER_MEMBER_ID)
												}));
									...
									{record.BUYER_SECOND_MEMBER_ID ? (
										<Link
											...
											onClick={() => {
												setQuery(record.TRSR_ID);
												const MemberData = getAllMembersbyId(record.BUYER_SECOND_MEMBER_ID);
												setIsShowEditMemberModal(true);
											}}
										>
										...
										<div className="row">
											<div className="col-sm-12">
												<div className="form-group">
													...
													{/* {console.log("FFFFF/FFFFFFFFFFFFFFFF", values.MEMBER_ID)} */}
													<Select
														placeholder="Select Member"
														options={memberList}
														value={values.MEMBER_ID}
