...
const CreateTransaction = () => {
    ...
    Axios.get(baseApiUrl + "paymentMode/list")
      .then((res) => {
        ...
        res.data.PaymentMode.map((item) => {
          setPaymentMode((prev) => [
            ...prev,
            { label: item.Description, value: item.PMID },
          ]);
        });
      })
      ...
      .then((res) => {
        ...
        res.data?.data
          ...
          .map((item) => {
            setNdcFee((prev) => [
              ...prev,
              { label: item.Name, value: item.NDC_ID, amount: item.Fee_Amt },
            ]);
          });
      })
      .catch((err) => console.log(err.response.data.message));
    ...
    Axios.get(baseApiUrl + "taxPayee/list")
      .then((res) => {
        res.data?.TaxPayeeCategorys.map((item) => {
          setTaxPayee((prev) => [
            ...prev,
            { label: item.Name, value: item.TPC_ID },
          ]);
        });
      })
      .catch((err) => console.log(err.response.data.message));
    ...
    Axios.get(baseApiUrl + "taxTag/list")
      .then((res) => {
        res.data?.TaxTags.map((item) => {
          setTaxTag((prev) => [
            ...prev,
            { label: item.Name, value: item.TT_ID, Amount: item.Amount },
          ]);
        });
      })
      .catch((err) => console.log(err.response.data.message));
      ...
      Axios.get(baseApiUrl + "file/getBookingByCode?id=" + url.split("=")[1])
        .then((res) => {
          if (res.data?.bkDetailObj) {
            ...
            let findDetailObj = res.data.files.find(
              (item) => item.data.BKI_DETAIL_ID == url.split("=")[1]
            );

            let RemainingAmt =
              parseFloat(findDetailObj?.data.Installment_Due) -
              parseFloat(getSumPaidAmt(findDetailObj));
  ...
  const handleChangePackage = (event, value, setFieldValueO) => {
    if (event.target.checked) {
      let dueAmt =
        parseFloat(value.data.Installment_Due) -
        parseFloat(getSumPaidAmt(value));
        ...
        {filteredData && (
          <Formik
            initialValues={{
              ...
              BK_ID: filteredData?.bkObj?.BK_ID
                ? filteredData?.bkObj?.BK_ID
                : "",
              ...filteredData,
            }}
            validate={(values) => {
            ...
            onSubmit={async (values, { setSubmitting }) => {
              const formData = {
                BKI_DETAIL_IDS:
                  values.BKI_DETAIL_IDS.length > 0
                    ? values.BKI_DETAIL_IDS
                    : detailIds,
                BK_ID: values.BK_ID,
              ...
              try {
                ...
                const res = await Axios.post(
                  baseApiUrl + "bookingTransaction/add",
                  formData
                );
                ...
                if (res.data.status == 200) {
                  ...
                  if (
                    ...
                    res.data.message.toLowerCase().includes("not")
                  ) {
                    toast.error(res.data.message);
                  } else if (
                    typeof res.data.message == "string" &&
                    res.data.message.toLowerCase().includes("!")
                  ) {
                                ...
                                <tr>
                                  <td>
                                    {it.data.InsType_ID != 3 && (
                                      <>
                                        {typeof window.location.href.split(
                                          "="
                                        )[1] != "undefined" ? (
                                          <input
                                            type="checkbox"
                                            checked={
                                              it.data.BKI_DETAIL_ID == detailId
                                                ? true
                                                : false
                                            }
                                          />
                                          ...
                                          <input
                                            ...
                                            disabled={
                                              parseFloat(
                                                it.data.Installment_Due
                                              ) -
                                                parseFloat(getSumPaidAmt(it)) <=
                                              0
                                                ? true
                                                : false
                                            }
