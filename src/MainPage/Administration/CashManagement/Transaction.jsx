...
const Transaction = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "Booking",
      sorter: (a, b) =>
        a?.Booking?.Reg_Code_Disply - b?.Booking?.Reg_Code_Disply,
      render: (text, record) => {
        return (
          <Space
            direction="horizontal"
  ...
  const columnsSearch = [
    ...
    },
    {
      ...
      sorter: (a, b) =>
        a.Reg_Code_Disply - b.Reg_Code_Disply,
      // render: (text, record) => {
      //   return (
      //     <Space
      //       direction="horizontal"
  ...
  const getAllTransactionbyBkReg = (Reg_Code_Disply) => {
    // Check and set CNIC
    ...
    if (Reg_Code_Disply.length > 0) {
      Axios.get(
        baseApiUrl + `transaction/BK_Reg_Code/list?Reg_Code_Disply=${Reg_Code_Disply}`
      )
        .then((res) => {
                ...
                {filteredRegData.map((itm, ind) => (
                  <tr>
                        ...
                        <div className="dropdown dropdown-action text-end">
                          ...
                          <div className="dropdown-menu dropdown-menu-right">
                            <Link
                              ...
                              onClick={() =>
                                TransactionReciept(itm.obj[0].INS_RC_ID)
                              }
                            >
                    ...
                    <td>
                      {planTypes &&
                        planTypes.find(
                          (item) => item?.InsType_ID == itm.data?.InsType_ID
                        )?.Name}
                    </td>
                    ...
                    <td>
                      {itm.obj.length > 0
                        ? itm.data.Installment_Due - getSumPaidAmt(itm)
                        : itm.data.Installment_Due}
                    </td>
                    <td>
                      {itm.obj.length > 0 ? "VCIRC-" + itm.obj[0].IRC_NO : ""}
                    </td>
                    <td>
                      {itm.obj.length > 0
                        ? itm.obj[0].Payment_Mode?.Description
                        : ""}
                    </td>
                    ...
                    <td>
                      {itm.data.Status === true && itm.data.InsType_ID  && itm.data.Installment_Due - getSumPaidAmt(itm) > 0 && (
                        <Link
                          to={
                            "/app/administrator/create-transaction?bki_id=" +
                            itm.data.BKI_DETAIL_ID
                          }
                        >
        ...
        <div className="modal-content">
          <div className="modal-header">
            ...
            <button
              type="button"
              className="close"
              onClick={() => setIsShowisShowDoubleObjectModal(false)}
            >
              <span aria-hidden="true">×</span>
            </button>
          </div>
          ...
          (
            ...
            <div className="modal-body">
              ...
              <table className="table table-stripped">
                ...
                {filteredObjData.obj.length > 0 && filteredObjData.obj?.map((itm, ind) => (
                  <tr>
                    <td>
                        ...
                        <div className="dropdown dropdown-action text-end">
                          ...
                          <div className="dropdown-menu dropdown-menu-right">
                            <Link
                              ...
                              data-bs-target="#edit_member"
                              onClick={() =>
                                TransactionReciept(itm.INS_RC_ID)
                              }
                            >
                              <i className="fa fa-download m-r-5" /> Download
                              PDF
                    ...
                    <td>
                      {planTypes &&
                        planTypes.find(
                          (item) => item?.InsType_ID == filteredObjData.data?.InsType_ID
                        )?.Name}
                    </td>
                    <td>{filteredObjData.data?.Due_Date}</td>
                    <td>{itm.Installment_Due}</td>
                    <td>
                      {itm.Installment_Paid}
                    </td>
                    <td>
                        {itm.Installment_Due - itm.Installment_Paid}
                    </td>
                    <td>
                      {"VCIRC-" + itm.IRC_NO}
                    </td>
                    <td>
                      {itm.Payment_Mode?.Description}
                    </td>

                    <td>
                      {itm.AdminVarified ? "Verified" : "Unverified"}
                    </td>
