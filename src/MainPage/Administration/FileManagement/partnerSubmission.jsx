...
const PartnerFileSubmission = () => {
  ...
  const [filesubmissionInitialValues, setFileSubmissionInitialValues] =
    useState({
      Form_Code: "",
      SRForm_No: "",
      Name: "",
  ...
  const getAllFileSubmission = (page, FSRC_Code, Name, CNIC) => {
    ...
    Axios.get(
      baseApiUrl +
        `fileSub/list?page=${page}&FSRC_Code=${FSRC_Code}&Name=${Name}&CNIC=${CNIC}`
    )
      .then((res) => {
    ...
    if (searchTerm.trim() === "") {
      ...
      Axios.get(baseApiUrl + "file/getFile?formNo=" + searchTerm)
        .then((res) => {
          ...
          const isFSRCDuplicate = fileSubmissionhit.some(
            (submission) => submission.formCode === item.Form_Code
          );
  ...
  const handleRemoveSubmission = (index) => {
    setFileSubmissionHit((prevSubmissions) =>
      prevSubmissions.filter((_, i) => i !== index)
    );
  };
                    ...
                    <div className="row">
                      ...
                      <div className="col-sm-6">
                        <div className="form-group">
                          ...
                          <span className="error">
                            {errors.Doc_Delivery_Date &&
                              touched.Doc_Delivery_Date &&
                              errors.Doc_Delivery_Date}
                          </span>
                        </div>
                      </div>
              ...
              onSubmit={async (values, { setSubmitting }) => {
                ...
                try {
                  ...
                  const res = await Axios.put(
                    baseApiUrl + `fileSub/update?id=${query}`,
                    formData
                  );
                  if (res.data.status == 200) {
...
export default PartnerFileSubmission;
