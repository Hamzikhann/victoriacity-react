...
const Users = () => {
  ...
  const getAllUsers = () => {
    Axios.get(baseApiUrl + "user/list")
      .then((res) => {
        ...
        setFilteredUser(
          res.data.user.filter((item) => item?.roles?.name !== "Super Admin")
        );
  ...
  const getRole = () => {
    Axios.get(baseApiUrl + "userRole/list")
      .then((res) => {
        res.data.UserRoles.filter(
          (item) => item.name !== "Super Admin" && item.name !== "Employee"
        ).map((item) => {
    ...
    Axios.get(baseApiUrl + "userGroup/list")
      .then((res) => {
        res.data.UserGroup.map((item) => {
          setGroup((prev) => [
            ...prev,
            { label: item.GROUP_NAME, value: item.Group_ID },
          ]);
        });
              ...
              <Link
                ...
                onClick={() => {
                  ...
                  setUserInitialValues({
                    ...
                    ...text,
                    role: roles.find(
                      (item) => item?.label === text?.roles?.name
                    ),
              ...
              onSubmit={async (values, { setSubmitting }) => {
                ...
                try {
                  ...
                  const res = await Axios.put(
                    baseApiUrl + `user/update?id=${query}`,
                    formData
                  );
                  if (res.data.status == 200) {
                    getAllUsers();
                      ...
                      <div className="col-sm-6">
                        <div className="form-group">
                          ...
                          <Select
                            placeholder="Select Role"
                            value={role?.find(
                              (item) => item?.label === values?.role?.label
                            )}
                        ...
                        <div className="form-group">
                          ...
                          <div>
                            <Select
                              placeholder="Select Groups"
                              value={group?.find(
                              (item) => item?.value == values?.user_group?.value
                            )}
                            // value={values.group}
                              options={group}
                              onChange={(value) => {
